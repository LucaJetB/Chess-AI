...
impl<'a> MoveGenerator<'a> {
    ...
    pub fn add_move(&mut self, dst: [i8;2], _dst_state: DesitinationState, player_color: Color) {
    ...
    pub fn get_castle(&mut self, dir: i8) -> Option<[i8;2]> { 
        ...
        for m in MoveGenerator::get_moves_with_predicates(self.board, self.piece.color.opposite_color(), false, |p| p != King, |_| true) {
    ...
    */
    ...
    pub fn get_moves_with_predicates(board: &Board, color: Color, defend: bool, piece_filter: impl Fn(Piece) -> bool, _dst_filter: impl Fn(DesitinationState) -> bool) -> Vec<Move> { 
        let mut results = Vec::new();
        for i in 0..8_i8 {
            for j in 0..8_i8 {
                match board.arr[i as usize][j as usize] { // ifmatch!
    ...
    pub fn get_moves(board: &Board, color: Color) -> Vec<Move> {
        Self::get_moves_with_predicates(board, color, false, |_| true, |_| true)
    }

    pub fn get_moves_with_defense(board: &Board, color: Color) -> Vec<Move> {
        Self::get_moves_with_predicates(board, color, true, |_| true, |_| true)
    }
